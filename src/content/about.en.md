...
## Everyone gets the same standard {#standard}
...
## Governance and editorial independence {#governance}
...
- Corrections are published, not quietly applied — every one of them is listed on [what has changed](/corrections), generated from that history rather than curated.
- No user accounts, no comments, no tracking. There is nothing here to compromise and nothing to leak.

## Who funds DeepTunisia {#funding}

## Current state, honestly {#state}

## How to read these numbers {#review-caveat}
...
## What the map does not contain {#coverage-intro}
...
## How to read the coverage table {#coverage-caveat}
...
If something here is wrong, the useful correction is a primary source: a decree number, a gazette reference, an official biography, a corporate filing. The [open questions](/evidence) are the current research agenda, and closing one of them is the fastest way to improve this map. The [open data](/data) page has the full graph, so anyone can check the reasoning rather than take it on trust. Everything that has already been changed is on [what has changed](/corrections).
